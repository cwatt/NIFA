...
```{r}
# Add number of substrates feature
# Calculate
...
clustering_df <- growth_incorp %>% 
  select(ASV:n16S, substrate, incorp_firstday, substrate_num) %>% # remove blast stuff
  mutate(incorp_firstday = as.character(incorp_firstday)) %>% # change to categorical
  pivot_wider(id_cols = c(ASV:n16S, substrate_num), names_from=substrate, values_from=incorp_firstday) %>% # wide format
...
```{r}
cor(clustering_df[, c('g', 'start_day', 'start_abund_corr', 'end_abund_corr', 'change_abund_corr', 'n16S', 'substrate_num')])
```
...
```{r, eval=FALSE}
saveRDS(clustering_df2, file="../data_intermediate/NIFA_cluster_features.rds")
```
