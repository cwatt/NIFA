...
```{r}
greff_df %>% 
  ggplot(aes(x=Day, y=log(greff), color=Soil)) +
  geom_point() +
  geom_smooth() +
  theme_test()
```

# PIN: How does this curve compare with shifts in incorporation profiles?

## Statistics

```{r}
greff_lm <- lm(log(greff) ~ Soil+Day+Soil*Day, data=greff_df)
hist(resid(greff_lm)) # not normal w/log transform
plot(predict(greff_lm), resid(greff_lm)) # homoskedastic w/log transform
# Switch to non-parametric test on log transformed data
kruskal.test(log(greff) ~ Soil, data=greff_df)
kruskal.test(log(greff) ~ Day, data=greff_df)
```
