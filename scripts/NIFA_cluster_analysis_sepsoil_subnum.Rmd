...
```{r, eval=FALSE}
ggsave(prop0_plot_isme_nolegend, file="../figures/fig_abundprop0_isme_nolegend.svg", units="mm", width=70, height=80, device="svg")
```
...
head(abund_total)
abund_total %>% 
  mutate(cluster_name = if_else(cluster == 1, "competitor", if_else(cluster==2, "scarcity", "ruderal"))) %>% 
  group_by(Soil, Day, cluster, cluster_name) %>% 
  summarize(cluster_total = mean(cluster_total)) %>% 
  ggplot(aes(x=Day, y=cluster_total, shape=cluster_name, color=cluster_name)) +
  geom_point() +
  facet_wrap(~Soil) +
  labs(y="total abundnace of growing taxa in cluster")
```
