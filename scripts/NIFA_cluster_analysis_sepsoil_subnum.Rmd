...
csgreff_plot_isme <- params_greffresp %>% 
  mutate(Soil = if_else(Soil=="Ag", "Agricultural", "Meadow")) %>%
  ggplot(aes(x=cs, y=log(greff))) +
  geom_point(aes(shape=Soil)) +
  scale_shape_manual(values=c(1,16)) +
...
```{r, eval=FALSE}
ggsave(csgreff_plot_isme, file="../figures/fig_csgreff.svg", units="mm", height=70, width=80, device="svg")
```
...
filter(csgreff_avg, Soil=="Meadow") %>% nrow()
```
...
```{r, eval=FALSE}
ggsave(csresp_plot, file="../figures/fig_csresp.svg", units="mm", height=70, width=80, device="svg")
```
...
filter(csgreff_avg, Soil=="Ag") %>% nrow()
```
