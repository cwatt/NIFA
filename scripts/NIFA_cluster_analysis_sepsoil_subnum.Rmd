...
```{r, eval=FALSE}
ggsave(diedprop_plot, file="../figures/fig_diedprop.svg", units="mm", height=180, width=150, device="svg")
ggsave(h_plot, file="../figures/fig_hclusters.svg", units="mm", height=90, width=150, device="svg")
ggsave(chabund_death_plot, file="../figures/fig_chabunddeathclusters.svg", units="mm", height=90, width=150, device="svg")
```
...
```{r, eval=FALSE}
ggsave(subnum_plot, file="../figures/fig_subnum.svg", units="mm", height=90, width=180, device="svg") 
```
...
```{r, eval=FALSE}
ggsave(subpref_plot, file="../figures/fig_subpref.svg", units="mm", height=90, width=180, device="svg")
```
...
```{r, eval=FALSE}
ggsave(phylo_plot, file="../figures/suppfig_phylomemb.svg", units="mm", height=180, width=120, device="svg")
```
...
```{r, eval=FALSE}
ggsave(abundtotal0_plot, file="../figures/fig_abundtotal0.svg", units="mm", width=180, height=90, device="svg")
```
...
```{r, eval=FALSE}
ggsave(prop0_plot, file="../figures/fig_abundprop0.svg", units="mm", width=180, height=90, device="svg")
```
...
```{r, eval=FALSE}
ggsave(rcsgreff_plot, file="../figures/fig_rcsgreff.svg", units="mm", height=90, width=180, device="svg")
```
...
```{r, eval=FALSE}
ggsave(rcsresp_plot, file="../figures/fig_rcsresp.svg", units="mm", height=90, width=180, device="svg")
```
...
```{r}
net_clust %>% 
  ggplot(aes(x=cluster, y=net, shape=Soil)) +
  geom_jitter(height=0, width=0.1) +
  scale_shape_manual(values=c(1,16)) +
  theme_test()
```
...
```{r}
net_lm <- lm(net ~ cluster*Soil, data=net_clust)
hist(resid(net_lm)) # not normal
plot(predict(net_lm), resid(net_lm)) # heteroskedastic, pick poison'
# Welch anova
oneway.test(net ~ Soil, data=net_clust, var.equal=FALSE)
oneway.test(net ~ cluster, data=net_clust, var.equal = FALSE)
oneway.test(net ~ cluster, data=filter(net_clust, Soil=="Ag"), var.equal = FALSE)
oneway.test(net ~ cluster, data=filter(net_clust, Soil=="Meadow"), var.equal = FALSE)
```
