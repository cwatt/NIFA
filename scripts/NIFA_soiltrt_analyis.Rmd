...
Total unique ASVs with growht/death estimates:
...
death_com_padj <- p.adjust(c(summary(g_de_lm)$coefficients[2,4], summary(lag_de_lm)$coefficients[2,4],
                             n16S_de_welch$p.value, chabund_de_kw$p.value, startabund_de_kw$p.value, endabund_de_welch$p.value),
                           method = "holm", n=6)
...
growth_death_asv %>% 
  ...
  ggplot(aes(x=log(growth), y=log(abs(death)), color=Soil)) +
  geom_point() +
